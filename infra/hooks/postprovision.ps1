...
try {
    ...
    $configContent = "NBK Speech-to-Speech Deployment Configuration$nl"
    $configContent += "Generated: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss')$nl$nl"
    $configContent += "FRONTEND CONFIGURATION$nl"
    $configContent += "================================================================$nl$nl"
    $configContent += "WebSocket URL:$nl"
    $configContent += "$fullWebsocketUrl$nl$nl"
    $configContent += "API Key:$nl"
    $configContent += "$subscriptionKey$nl$nl"
    $configContent += "Authentication:$nl"
    $configContent += "Add header: api-key: YOUR_API_KEY_HERE$nl$nl"
    $configContent += "DEPLOYMENT DETAILS$nl"
    $configContent += "================================================================$nl$nl"
    $configContent += "Resource Group: $resourceGroup$nl"
    $configContent += "Region: $region$nl"
    $configContent += "API Version: $apiVersion$nl"
    $configContent += "APIM Gateway: $gatewayUrl$nl$nl"
    $configContent += "FEATURES$nl"
    $configContent += "================================================================$nl$nl"
    $configContent += "* Speech-to-Speech (bidirectional audio)$nl"
    $configContent += "* Arabic and English support$nl"
    $configContent += "* NBK knowledge base (web scraping)$nl"
    $configContent += "* Real-time interruption support$nl"
    $configContent += "* Professional 'echo' voice$nl$nl"
    $configContent += "Integration examples: see FRONTEND.md$nl"
